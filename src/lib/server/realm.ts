...
export async function lookupRealm(userId: string): Promise<string | null> {
	try {
		const res = await fetch(`${env.REALM_SERVICE_URL}/${encodeURIComponent(userId)}`);
 ...
 * Key pattern: ${REDIS_KEY_PREFIX}:${KEYCLOAK_CLIENT_ID}:<realmId>
 */
export async function getClientSecret(realmId: string): Promise<string | null> {
	const key = `${env.REDIS_KEY_PREFIX}:${env.KEYCLOAK_CLIENT_ID}:${realmId}`;
...
export function getKeycloakIssuer(realmId: string): string {
	return `${env.KEYCLOAK_URL}/realms/${realmId}`;
}
